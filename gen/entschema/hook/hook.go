...
// The PhotographCityFunc type is an adapter to allow the use of ordinary
// function as PhotographCity mutator.
type PhotographCityFunc func(context.Context, *entschema.PhotographCityMutation) (entschema.Value, error)

// Mutate calls f(ctx, m).
func (f PhotographCityFunc) Mutate(ctx context.Context, m entschema.Mutation) (entschema.Value, error) {
	if mv, ok := m.(*entschema.PhotographCityMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *entschema.PhotographCityMutation", m)
}
